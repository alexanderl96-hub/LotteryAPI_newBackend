...
// GET all loto_plus entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM loto_plus ORDER BY id DESC");
...
// GET loto_plus entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM loto_plus WHERE id = $1`, [id]);
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM loto_plus WHERE id = $1`, [id]);
