...
// GET all tripletwist entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM tripletwist ORDER BY id DESC");
    res.json({ status: 200, data_tripletwist: result });
  } catch (error) {
...
// GET tripletwist entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM tripletwist WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10TripleTwist = await db.any(`
        SELECT *
        FROM tripletwist
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10TripleTwist });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM tripletwist WHERE id = $1`, [id]);
