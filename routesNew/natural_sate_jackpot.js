...
// GET all natural_state_jackpot entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM natural_state_jackpot ORDER BY id DESC");
...
// GET natural_state_jackpot entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM natural_state_jackpot WHERE id = $1`, [id]);
...
// POST new natural_state_jackpot entry
router.post('/', async (req, res) => {
    try {
      ...
      const insertQuery = `
        INSERT INTO natural_state_jackpot(
          date,
          nextDrawDate,
          nextDrawJackpot,
          numbers,
      ...
      // fetch last 10 rows
      const last10NaturalStateJackpot = await db.any(`
        SELECT *
        FROM natural_state_jackpot
        ORDER BY id DESC
        LIMIT 10
      `);
  
      res.json({ status: 200, last10Draws: last10NaturalStateJackpot});
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM natural_state_jackpot WHERE id = $1`, [id]);
