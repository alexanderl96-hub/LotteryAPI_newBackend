...
// GET all take5 entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM take5 ORDER BY id DESC");
    res.json({ status: 200, data_take5: result });
  } catch (error) {
...
// GET take5 entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM take5 WHERE id = $1`, [id]);
...
//       // fetch last 10 rows
//       const last10Take5 = await db.any(`
//         SELECT *
//         FROM take5
//         ORDER BY id DESC
//         LIMIT 10
...
//       res.json({ status: 200, last10Draws: last10Take5 });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM take5 WHERE id = $1`, [id]);
