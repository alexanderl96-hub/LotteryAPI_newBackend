...
// GET all fantasy_5_ga entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM fantasy_5_ga ORDER BY id DESC");
    res.json({ status: 200, data_fantasy_5_ga: result });
  } catch (error) {
...
router.get('/:id', async (req, res) => {
    try {
      ...
      const draw = await db.oneOrNone(`SELECT * FROM fantasy_5_ga WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10fantasy_5_ga = await db.any(`
        SELECT *
        FROM fantasy_5_ga
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10fantasy_5_ga });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM fantasy_5_ga WHERE id = $1`, [id]);
