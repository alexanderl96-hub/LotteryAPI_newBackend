...
// GET all bank_a_million  entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM bank_a_million  ORDER BY id DESC");
    res.json({ status: 200, data_bank_a_million : result });
  } catch (error) {
...
// GET bank_a_million  entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM bank_a_million  WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10bank_a_million = await db.any(`
        SELECT *
        FROM bank_a_million 
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10bank_a_million});
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM bank_a_million  WHERE id = $1`, [id]);
