...
// GET all play_5 entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM play_5 ORDER BY id DESC");
    res.json({ status: 200, data_play_5: result });
  } catch (error) {
...
// GET play_5 entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM play_5 WHERE id = $1`, [id]);
...
//       // fetch last 10 rows
//       const last10Play_5 = await db.any(`
//         SELECT *
//         FROM play_5
//         ORDER BY id DESC
//         LIMIT 10
...
//       res.json({ status: 200, last10Draws: last10Play_5 });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM play_5 WHERE id = $1`, [id]);
