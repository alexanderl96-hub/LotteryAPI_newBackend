...
// GET all play4 entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM play4 ORDER BY id DESC");
    res.json({ status: 200, data_play4: result });
  } catch (error) {
...
// GET play4 entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM play4 WHERE id = $1`, [id]);
...
//       // fetch last 10 rows
//       const last10Play4 = await db.any(`
//         SELECT *
//         FROM play4
//         ORDER BY id DESC
//         LIMIT 10
...
//       res.json({ status: 200, last10Draws: last10Play4 });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM play4 WHERE id = $1`, [id]);
