...
// GET all win_for_life  entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM win_for_life  ORDER BY id DESC");
    res.json({ status: 200, data_Win_For_Life : result });
  } catch (error) {
...
// GET win_for_life  entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM win_for_life  WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10WinForLife  = await db.any(`
        SELECT *
        FROM win_for_life 
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10WinForLife });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM win_for_life  WHERE id = $1`, [id]);
