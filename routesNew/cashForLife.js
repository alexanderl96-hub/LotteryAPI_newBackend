...
// GET all cashforlife entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM cashforlife ORDER BY id DESC");
    res.json({ status: 200, data_cashforlife: result });
  } catch (error) {
...
// GET cashforlife entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM cashforlife WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10CashForlIfe = await db.any(`
        SELECT *
        FROM cashforlife
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10CashForlIfe });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM cashforlife WHERE id = $1`, [id]);
