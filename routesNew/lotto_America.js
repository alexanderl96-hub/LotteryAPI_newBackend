...
// GET all lotto_america entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM lotto_america ORDER BY id DESC");
...
// GET lotto_america entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM lotto_america WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10LottoAmerica = await db.any(`
        SELECT *
        FROM lotto_america
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10LottoAmerica });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM lotto_america WHERE id = $1`, [id]);
