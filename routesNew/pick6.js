...
// GET all pick_6 entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM pick_6 ORDER BY id DESC");
    res.json({ status: 200, data_pick_6: result });
  } catch (error) {
...
// GET pick_6 entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM pick_6 WHERE id = $1`, [id]);
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM pick_6 WHERE id = $1`, [id]);
