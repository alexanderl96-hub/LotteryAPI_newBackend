...
// GET all cash_25  entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM cash_25  ORDER BY id DESC");
...
// GET cash_25  entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM cash_25  WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10cash_25 = await db.any(`
        SELECT *
        FROM cash_25 
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10cash_25});
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM cash_25  WHERE id = $1`, [id]);
