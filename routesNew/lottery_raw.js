...
const storedData = require('../ServiceUpdate/dataJsonAPi_Request');
...
router.get('/:id', async (req, res) => {
    try {
      ...
      const draw = await db.oneOrNone(`SELECT * FROM remaindata WHERE id = $1`, [id]);
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM remaindata WHERE id = $1`, [id]);
