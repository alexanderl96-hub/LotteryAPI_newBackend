...
// GET all wild_money  entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM wild_money  ORDER BY id DESC");
...
// GET wild_money  entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM wild_money  WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10wild_money = await db.any(`
        SELECT *
        FROM wild_money 
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10wild_money});
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM wild_money  WHERE id = $1`, [id]);
