...
// GET all mega millions entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM mega_millions ORDER BY id DESC");
    res.json({ status: 200, data_megamillions: result });
  } catch (error) {
...
// GET mega millions entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM mega_millions WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10MegaMillions = await db.any(`
        SELECT *
        FROM mega_millions
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10MegaMillions });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM mega_millions WHERE id = $1`, [id]);
