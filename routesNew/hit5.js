...
// GET all hit_5  entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM hit_5  ORDER BY id DESC");
    res.json({ status: 200, data_hit_5: result });
  } catch (error) {
...
// GET hit_5  entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM hit_5  WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      res.json({ status: 200, last10Draws: last10hit_5});
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM hit_5  WHERE id = $1`, [id]);
