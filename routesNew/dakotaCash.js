...
// GET all dakota_cash  entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM dakota_cash  ORDER BY id DESC");
...
// GET dakota_cash  entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM dakota_cash  WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10dakota_cash  = await db.any(`
        SELECT *
        FROM dakota_cash 
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10dakota_cash });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM dakota_cash  WHERE id = $1`, [id]);
