...
// GET all gimme_five entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM gimme_five ORDER BY id DESC");
    res.json({ status: 200, data_Gimme_Five: result });
  } catch (error) {
...
// GET gimme_five entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM gimme_five WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10Gimme_Five = await db.any(`
        SELECT *
        FROM gimme_five
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10Gimme_Five });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM gimme_five WHERE id = $1`, [id]);
