...
// GET all lucky_for_life entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM lucky_for_life ORDER BY id DESC");
...
// GET lucky_for_life entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM lucky_for_life WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10LuckyForLife = await db.any(`
        SELECT *
        FROM lucky_for_life
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10LuckyForLife});
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM lucky_for_life WHERE id = $1`, [id]);
