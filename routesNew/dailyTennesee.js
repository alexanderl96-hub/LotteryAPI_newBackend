...
// GET all daily_tennessee  entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM daily_tennessee  ORDER BY id DESC");
    res.json({ status: 200, data_daily_tennessee: result });
  } catch (error) {
...
// GET daily_tennessee  entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM daily_tennessee  WHERE id = $1`, [id]);
...
// POST new daily_tennessee  entry
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10daily_tennessee = await db.any(`
        SELECT *
        FROM daily_tennessee 
        ORDER BY id DESC
        LIMIT 10
      `);
  
      res.json({ status: 200, last10Draws: last10daily_tennessee});
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM daily_tennessee  WHERE id = $1`, [id]);
