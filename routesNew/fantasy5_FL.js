...
// GET all fantasy_5_fl entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM fantasy_5_fl ORDER BY id DESC");
    res.json({ status: 200, data_fantasy_5_fl: result });
  } catch (error) {
...
// GET fantasy_5_fl entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM fantasy_5_fl WHERE id = $1`, [id]);
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM fantasy_5_fl WHERE id = $1`, [id]);
