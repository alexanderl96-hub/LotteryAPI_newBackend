...
// GET all daily_game  entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM daily_game  ORDER BY id DESC");
    res.json({ status: 200, data_daily_game: result });
  } catch (error) {
...
// GET daily_game  entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM daily_game  WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10daily_game = await db.any(`
        SELECT *
        FROM daily_game 
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10daily_game});
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM daily_game  WHERE id = $1`, [id]);
