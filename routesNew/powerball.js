...
// GET all powerball entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM powerball ORDER BY id DESC");
...
// GET powerball entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM powerball WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10Powerball = await db.any(`
        SELECT *
        FROM powerball
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10Powerball });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM powerball WHERE id = $1`, [id]);
