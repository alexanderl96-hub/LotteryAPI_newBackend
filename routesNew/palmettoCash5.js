...
// GET all palmetto_cash_5  entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM palmetto_cash_5  ORDER BY id DESC");
    res.json({ status: 200, data_wild_money : result });
  } catch (error) {
...
// GET palmetto_cash_5  entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM palmetto_cash_5  WHERE id = $1`, [id]);
...
// POST new palmetto_cash_5  entry
router.post('/', async (req, res) => {
    try {
      ...
      const last10palmetto_cash_5 = await db.any(`
        SELECT *
        FROM palmetto_cash_5 
        ORDER BY id DESC
        LIMIT 10
      `);
  
      res.json({ status: 200, last10Draws: last10palmetto_cash_5});
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM palmetto_cash_5  WHERE id = $1`, [id]);
