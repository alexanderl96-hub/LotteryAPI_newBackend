...
// GET all treasure_hunt entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM treasure_hunt ORDER BY id DESC");
    res.json({ status: 200, data_treasure_hunt: result });
  } catch (error) {
...
// GET treasure_hunt entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM treasure_hunt WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10treasure_hunt = await db.any(`
        SELECT *
        FROM treasure_hunt
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10treasure_hunt });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM treasure_hunt WHERE id = $1`, [id]);
