...
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM dc2 ORDER BY id DESC");
    res.json({ status: 200, data_dc2: result });
  } catch (error) {
...
router.get('/:id', async (req, res) => {
    try {
      ...
      const draw = await db.oneOrNone(`SELECT * FROM dc2 WHERE id = $1`, [id]);
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM dc2 WHERE id = $1`, [id]);
