...
// GET all play_3 entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM play_3 ORDER BY id DESC");
    res.json({ status: 200, data_play_3: result });
  } catch (error) {
...
// GET play_3 entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM play_3 WHERE id = $1`, [id]);
...
//       // fetch last 10 rows
//       const last10Play_3 = await db.any(`
//         SELECT *
//         FROM play_3
//         ORDER BY id DESC
//         LIMIT 10
...
//       res.json({ status: 200, last10Draws: last10Play_3 });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM play_3 WHERE id = $1`, [id]);
