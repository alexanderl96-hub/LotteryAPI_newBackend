...
// GET all match_4  entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM match_4  ORDER BY id DESC");
    res.json({ status: 200, data_match_4: result });
  } catch (error) {
...
// GET match_4  entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM match_4  WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10match_4 = await db.any(`
        SELECT *
        FROM match_4 
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10match_4});
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM match_4  WHERE id = $1`, [id]);
