...
// GET all pick10 entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM pick10 ORDER BY id DESC");
    res.json({ status: 200, data_pick10: result });
  } catch (error) {
...
// GET pick10 entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM pick10 WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10Pick10 = await db.any(`
        SELECT *
        FROM pick10
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10Pick10 });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM pick10 WHERE id = $1`, [id]);
