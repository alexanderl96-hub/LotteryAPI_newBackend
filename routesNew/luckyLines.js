...
// // GET all lucky_lines entries
// router.get("/", async (req, res) => {
//   try {
//     const result = await db.any("SELECT * FROM lucky_lines ORDER BY id DESC");
//     res.json({ status: 200, data_Lucky_Lines: result });
//   } catch (error) {
...
// // GET lucky_lines entry by ID
// router.get('/:id', async (req, res) => {
//     try {
//       const id = parseInt(req.params.id);
//       const draw = await db.oneOrNone(`SELECT * FROM lucky_lines WHERE id = $1`, [id]);
...
//       // fetch last 10 rows
//       const last10LuckyLines = await db.any(`
//         SELECT *
//         FROM lucky_lines
//         ORDER BY id DESC
//         LIMIT 10
...
//       res.json({ status: 200, last10Draws: last10LuckyLines });
...
//     try {
//       const id = parseInt(req.params.id);
//       const result = await db.result(`DELETE FROM lucky_lines WHERE id = $1`, [id]);
