...
// GET all match_6_lotto entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM match_6_lotto ORDER BY id DESC");
    res.json({ status: 200, data_match_6_lotto: result });
  } catch (error) {
...
// GET match_6_lotto entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM match_6_lotto WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10match_6_lotto = await db.any(`
        SELECT *
        FROM match_6_lotto
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10match_6_lotto });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM match_6_lotto WHERE id = $1`, [id]);
