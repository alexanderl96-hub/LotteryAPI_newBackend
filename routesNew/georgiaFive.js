...
// GET all georgia_five entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM georgia_five ORDER BY id DESC");
    res.json({ status: 200, data_georgia_five: result });
  } catch (error) {
...
// GET georgia_five entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM georgia_five WHERE id = $1`, [id]);
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM georgia_five WHERE id = $1`, [id]);
