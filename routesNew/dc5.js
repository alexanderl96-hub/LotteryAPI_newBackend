...
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM dc5 ORDER BY id DESC");
    res.json({ status: 200, data_dc5: result });
  } catch (error) {
...
router.get('/:id', async (req, res) => {
    try {
      ...
      const draw = await db.oneOrNone(`SELECT * FROM dc5 WHERE id = $1`, [id]);
...
//       res.json({ status: 200, last10Draws: last10DC5 });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM dc5 WHERE id = $1`, [id]);
