...
// GET all jackpotTriplePlay entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM jackpotTriplePlay ORDER BY id DESC");
    res.json({ status: 200, data_jackpotTriplePlay: result });
  } catch (error) {
...
// GET jackpotTriplePlay entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM jackpotTriplePlay WHERE id = $1`, [id]);
...
// POST new jackpotTriplePlay entry
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10JackpotTriplePlay = await db.any(`
        SELECT *
        FROM jackpotTriplePlay
        ORDER BY id DESC
        LIMIT 10
      `);
  
      res.json({ status: 200, last10Draws: last10JackpotTriplePlay });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM jackpotTriplePlay WHERE id = $1`, [id]);
