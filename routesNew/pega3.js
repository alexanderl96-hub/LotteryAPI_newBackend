...
// GET all pega_3 entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM pega_3 ORDER BY id DESC");
...
// GET pega_3 entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM pega_3 WHERE id = $1`, [id]);
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM pega_3 WHERE id = $1`, [id]);
