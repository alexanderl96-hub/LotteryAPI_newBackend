...
// GET all megabucks_plus entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM megabucks_plus ORDER BY id DESC");
    res.json({ status: 200, data_Megabucks_Plus: result });
  } catch (error) {
...
// GET megabucks_plus entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM megabucks_plus WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10Megabucks_Plus = await db.any(`
        SELECT *
        FROM megabucks_plus
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10Megabucks_Plus });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM megabucks_plus WHERE id = $1`, [id]);
