...
// GET all fantasy_5_mi entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM fantasy_5_mi ORDER BY id DESC");
    res.json({ status: 200, data_fantasy_5_mi: result });
  } catch (error) {
...
// GET fantasy_5_mi entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM fantasy_5_mi WHERE id = $1`, [id]);
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM fantasy_5_mi WHERE id = $1`, [id]);
