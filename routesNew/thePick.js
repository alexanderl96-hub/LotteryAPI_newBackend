...
// GET all thepick entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM thepick ORDER BY id DESC");
    res.json({ status: 200, data_thepick: result });
  } catch (error) {
...
// GET thepick entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM thepick WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10ThePick = await db.any(`
        SELECT *
        FROM thepick
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10ThePick });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM thepick WHERE id = $1`, [id]);
