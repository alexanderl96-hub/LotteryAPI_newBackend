...
// GET all luckydaylotto entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM luckydaylotto ORDER BY id DESC");
    res.json({ status: 200, data_luckydaylotto: result });
  } catch (error) {
...
// GET luckydaylotto entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM luckydaylotto WHERE id = $1`, [id]);
...
//       // fetch last 10 rows
//       const last10LuckydayLotto = await db.any(`
//         SELECT *
//         FROM luckydaylotto
//         ORDER BY id DESC
//         LIMIT 10
...
//       res.json({ status: 200, last10Draws: last10LuckydayLotto });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM luckydaylotto WHERE id = $1`, [id]);
