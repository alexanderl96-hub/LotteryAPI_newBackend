...
// GET all pega_4 entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM pega_4 ORDER BY id DESC");
...
// GET pega_4 entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM pega_4 WHERE id = $1`, [id]);
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM pega_4 WHERE id = $1`, [id]);
