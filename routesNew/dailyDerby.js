...
// GET all dailyderby entries
router.get("/", async (req, res) => {
  try {
    const result = await db.any("SELECT * FROM dailyderby ORDER BY id DESC");
    res.json({ status: 200, data_dailyderby: result });
  } catch (error) {
...
// GET dailyderby entry by ID
router.get('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const draw = await db.oneOrNone(`SELECT * FROM dailyderby WHERE id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
      ...
      // fetch last 10 rows
      const last10DailyDerby = await db.any(`
        SELECT *
        FROM dailyderby
        ORDER BY id DESC
        LIMIT 10
      ...
      res.json({ status: 200, last10Draws: last10DailyDerby });
...
router.delete('/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const result = await db.result(`DELETE FROM dailyderby WHERE id = $1`, [id]);
