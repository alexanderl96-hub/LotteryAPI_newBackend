...
router.post('/', async (req, res) => {
    try {
        ...
        const addingNewData = await db.one(
            `INSERT INTO generate_numbers_megamillions (date, storage, amount) 
...
router.delete('/:id', async (req, res) => {
    try {
        const element = await db.one('DELETE FROM generate_numbers_megamillions WHERE id = $1', [req.params.id]);
        res.json(element)
        res.send({
