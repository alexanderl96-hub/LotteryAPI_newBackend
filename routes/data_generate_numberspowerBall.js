...
router.get('/', async function (req, res, next) {
    try {
        const all_data = await db.any('SELECT * FROM generate_numbers_powerball');
        res.json({data_generateNPowerball : all_data});
    } catch (error) {
        res.send({
            status : 404,
...
router.post('/', async (req, res) => {
    try {
        ...
        const addingNewData = await db.one(
            `INSERT INTO generate_numbers_powerball (date, storage, amount) 
...
router.delete('/:id', async (req, res) => {
    try {
        const element = await db.one('DELETE FROM generate_numbers_powerball WHERE id = $1', [req.params.id]);
        res.json(element)
        res.send({
