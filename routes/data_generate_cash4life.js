...
router.get('/', async function (req, res, next) {
    try {
        const all_data = await db.any('SELECT * FROM generate_numbers_cashforlife');
        res.json({data_generateNCashForLife : all_data});
    } catch (error) {
        res.send({
            status : 404,
...
router.post('/', async (req, res) => {
    try {
        ...
        const addingNewData = await db.one(
            `INSERT INTO generate_numbers_cashforlife (date, storage, amount) 
...
router.delete('/:id', async (req, res) => {
    try {
        const element = await db.one('DELETE FROM generate_numbers_cashforlife WHERE id = $1', [req.params.id]);
        res.json(element)
        res.send({
