...
const saveLotteryToDb = async (data) => {
    try {
        const query = `
            INSERT INTO lottery_raw(draw_data)
            VALUES($1)
            RETURNING id
        `;
...
router.get('/', async (req, res, next) => {
    try {
        ...
        // Save to database
        // await saveLotteryToDb(all_data);

        res.json({ status: "success", dataSelect: selectedGames, data: remainingData });
    } catch (error) {
